...
- [3. Operator / Delegator Distribution](#3-operator--delegator-distribution)
  ...
  - [3.3 Problem Induction → Collapse the two thresholds, simplify the game](#33-problem-induction--collapse-the-two-thresholds-simplify-the-game)
...
The intra-pool split analysis (epochs 211–615) yields ten observations at this pipeline stage. The full data, figures, entity analysis, and reproduction scripts are in the dedicated sub-report: [*The Operator's Cut*](sub-flows/operator-delegator-distribution/mainnet-analysis/README.md).

| # | Observation | Summary |
| --- | --- | --- |
| **O1** | **The fixed-cost floor exhibits striking inertia** | The protocol floor $c_{\min}$ was reduced from 340 to 170 ADA at epoch 445 (2023/10/27). 169 epochs later, 564 pools (64.5%) still declare 340 ADA — the former floor. Only 217 pools (24.8%) have adopted the current floor. 89.3% of pools sit at one of the two floor values. The 564-pool residual has a material yield cost for small pools — a pool at 340 ADA pays double the minimum, eroding delegator yield by an amount that scales inversely with pool size. |
| **O2** | **Margin competition is active but masked by a high-margin tail** | Margin is bimodal: 50.7% of pools declare ≤ 2%, while 11.8% declare ≥ 99% (private + functionally private). The median margin has been stable at 2.0% for 405 epochs, but the stake-weighted mean has risen from 4.2% to 18.9% — driven by the growing weight of private and captive pools, not by fee increases in the competitive market. |
...
| **O7** | **The delegator yield is declining on a predictable trajectory** | Stake-weighted hollow-market yield at epoch 614: 2.01% annualised (≈201 ADA/year per 10k ADA). Yield tracks reserve depletion with R²=0.99 and halves roughly every 3 years. Projected threshold crossings: < 2% in ~0.4yr, < 1.5% in ~1.7yr, < 1% in ~3.5yr, < 0.5% in ~6.7yr. |
| **O8** | **Cardano's yield sits below the risk-free rate and most PoS peers** | Among major PoS chains, only the S&P 500 dividend yield sits below Cardano's staking return; Ethereum delivers 1.5–2×, higher-inflation chains 3–10×. What Cardano loses in yield it gains in design: no lockup, no slashing, no minimum, no custodial transfer — unique among major PoS chains. |
| **O9** | **The yield spread between well-run pools is narrow** | Hollow and balanced yields correlate at 0.97 over 405 epochs — both strategies track reserve depletion in lockstep. In the 30–77M bucket (70% of hollow delegation), the middle-half spread is just 0.46pp; 70.2% of delegated stake sits within ±0.5pp of the median. SPO vs MPO yield difference is negligible (2.05% vs 2.00%). |
| **O10** | **Fixed cost, not margin, is the dominant yield differentiator** | The fixed-cost floor acts as a regressive levy: 54.3% effective tax for sub-3M pools, collapsing to 4.7% in the 30–77M band. A counterfactual removing the floor flattens the yield surface entirely — median yield rises +1.0pp for < 3M pools but only +0.05pp for 30–77M pools. The aggregate fixed-cost share has tripled from 1.6% (epoch 250) to 4.9% (epoch 614) and will continue rising. |
| **O11** | **Block luck dominates single-epoch variance; structure emerges only over time** | Block-production luck explains R²=0.41 of single-epoch yield variance among large hollow pools. Over a full year, the structural spread is an order of magnitude smaller than single-epoch noise (hollow SW std dev: 0.10pp across 73 trailing epochs). |

> **Scope note.** O1–O2 cover fee parameter adoption. O3–O6 are structural to the intra-pool split. O7–O11 characterise the delegator's yield landscape. The fixed-cost regressive tax (O5, O10) and the flat yield surface (O9) have the most direct implications for mechanism revision.

## 3.3 Problem Induction → Collapse the two thresholds, simplify the game

Each observation above constrains what the fee structure can accomplish. Read together, they reveal a mechanism that operates against its own goals — and point toward a concrete structural reform.

**The fee structure creates a dead zone.** The fixed-cost floor ($minPoolCost$) acts as a regressive tax that penalises small pools disproportionately (O5, O10). At sub-3M pools, the effective tax exceeds 54%; at the production threshold (~1M ₳), it approaches 100%. This is the mechanism that *creates* the viability threshold: without the floor, a pool that produces blocks is immediately economically viable — the counterfactual in O10 demonstrates this directly, flattening the yield surface entirely. The corridor between the production threshold (~1M ₳, set by Poisson physics) and the viability threshold (~3M ₳, set by the fee structure) is an artefact of the fixed-cost floor, not a structural necessity.

**The floor's burden is growing.** As the reserve depletes and the epoch pot shrinks, the fixed-cost share of pool rewards rises mechanically — from 1.6% at epoch 250 to 4.9% at epoch 614, and accelerating (O10). The hyperbolic penalty that today affects sub-3M pools will, within a few years, erode yields for pools in the 5–10M range that are currently viable. The dead zone is not static; it is expanding upward.

**Margin competition is healthy — the fixed cost is the distortion.** The median margin has been stable at 2.0% for 405 epochs (O2). Among large pools, margin differences cost the delegator only 0.07pp per 3% of margin — negligible (O10). The yield surface is flat (O9) because the reward curve distributes rewards roughly proportional to stake. It is the fixed-cost floor that breaks this flatness, creating a size gradient that has nothing to do with pool quality, operator commitment, or service level. Removing or drastically reducing the floor would preserve the healthy margin competition while eliminating the regressive distortion.

**The balanced strategy is doubly penalised.** Balanced-strategy pools tend to be smaller than hollow pools — and the fixed cost hits them harder (O6). The population that the pledge mechanism is supposed to reward (§2.4.4.2) is the one most damaged by the fee structure. A revised reward curve that incentivises balanced operators will underperform if the fee structure simultaneously taxes their smaller pools at a higher rate. The two pipeline stages must be reformed in concert.

**The structural conclusion: collapse the two thresholds into one.** §2.4.4.1 identifies two implicit thresholds that a prospective operator must cross — the production floor and the viability floor — and proposes enforcing the production threshold explicitly ($\sigma_{\min}$). The intra-pool split analysis completes the argument: the corridor between the two thresholds is created entirely by the fixed-cost floor. Reducing $minPoolCost$ to zero (or to a negligible value that tracks the reward curve) collapses the viability threshold down to the production threshold. The result:

- **One threshold, one gate.** Below $\sigma_{\min}$, the protocol does not permit pool registration — and the Rocket Pool-like pooling service (§2.4.4.1) operates as the legitimate sub-threshold path. At $\sigma_{\min}$, the operator is immediately economically viable. No dead zone, no misleading corridor, no regressive tax.
- **A flat yield surface from entry to saturation.** Once the fixed-cost distortion is removed, delegator yield depends on margin and block production — both of which are proportional to stake. The size gradient disappears. A 3M pool and a 30M pool offer near-identical yield, differentiated only by margin choice and block-production variance (which averages out over time — O11). Pool selection becomes about commitment and quality, not about avoiding the fixed-cost penalty.
- **The balanced strategy is no longer penalised by its size.** Small balanced pools — the population the mechanism should nurture — are freed from the regressive tax that currently erodes their competitiveness against larger hollow pools.

This reform interacts directly with the reward-curve revision proposed in §2.4.4.2. A revised curve that makes pledge economically significant, combined with a fee structure that does not penalise the pools where pledge is most prevalent, creates a coherent incentive gradient: the balanced path is rewarding at every scale, from production threshold to saturation. The two pipeline stages — reward curve and intra-pool split — must be designed as a single system, not as independent layers.

Whether $minPoolCost$ should be set to zero outright or replaced by a proportional mechanism (e.g., a percentage-based minimum that scales with pool size) is a design question that simulation and governance must resolve. The analytical conclusion is unambiguous: the fixed-cost *floor* as currently structured is the single largest addressable distortion in the fee layer, and its removal is a prerequisite for the legible, one-threshold game that §2.4.4.1 envisions.
