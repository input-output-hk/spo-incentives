...
| CIP-0050.S1.F1 | [D] | **Zero-pledge hard break.** $L \cdot 0 = 0$: any pool with $\pi = 0$ has $\sigma' = 0$ and earns zero reward regardless of delegation. Restores pledge as a binding constraint — direct delivery on V2 §3.2 pledge-as-signal | §2.2, §3.1 |
...
The reward curve is not expressed in $\sigma$ and $p$ directly but in two **normalised coordinates** (from pools-distribution §2.3):

- **$\pi = p / z_0$ — pledge saturation level:** what fraction of the saturation threshold is covered by operator pledge. $\pi = 0$ means no pledge; $\pi = 1$ means pledge alone equals the saturation threshold.
- **$\nu = \sigma / z_0$ — total-stake saturation level:** what fraction of the saturation threshold is covered by the pool's total stake. $\nu = 1$ means the pool is at V1 saturation; $\nu > 1$ means oversaturated. Below saturation, $0 < \pi \leq \nu \leq 1$.

The pool reward is then $\hat f'(\pi, \nu, \bar p) = \bar p \cdot P_{\max} \cdot E(\pi, \nu)$ where $P_{\max} = R/k$ (reward ceiling) and $E(\pi, \nu)$ is the envelope function (see [pools-distribution §2.3](../../diagnostic/sub-flows/pools-distribution/mainnet-analysis/README.md#23-reward-function)).

**CIP-0050 in normalised coordinates.** Dividing the CIP formula by $z_0$:

$$\nu'^{(50)}_{i} = \min\!\left(\nu_{i},\ 1,\ L \cdot \pi_{i}\right)$$

Read this as: *"the effective total-stake saturation level $\nu'$ is the smallest of (a) the pool's actual total-stake saturation level $\nu$, (b) V1 saturation 1, or (c) $L$ times the pool's pledge saturation level $\pi$"*. Only $\nu$ is potentially clipped — the pledge saturation $\pi$ is unchanged by the rule. The reward becomes $\hat f' = \bar p \cdot P_{\max} \cdot E(\pi, \nu')$.
...
| $L \cdot \pi$ (or $L \cdot p$) | **New** — cap proportional to pledge saturation level | When pledge ratio is too low: $L \cdot \pi < \nu$, equivalently $\pi / \nu < 1/L$, equivalently the within-pool pledge ratio $p/\sigma < 1/L$ |

For $L = 100$: the new cap binds whenever the pool's **within-pool pledge ratio** $p/\sigma = \pi/\nu$ falls below **1 %**. Above 1 %, CIP-0050 is invisible; at or below 1 %, the effective saturation level $\nu'$ is clipped to $100 \cdot \pi$ and the reward shrinks accordingly.
...
The three min-arguments ($\nu$, 1, $L\cdot\pi$) — equivalently $(\sigma, 1/k, L\cdot p)$ in absolute-supply units — carve the state space into three regimes. Each scenario below shows the four quantities $(\sigma, p, \pi, \nu)$ alongside the binding cap. $z_0 \approx 77$ M ADA at today's mainnet, L = 100.

**Scenario A — Compliant small pool (within-pool pledge ratio $p/\sigma \geq 1/L = 1\,\%$).**

| Quantity | Value |
| --- | --- |
| Total stake $\sigma_{\text{abs}}$ | 5 M ₳ |
| Pledge $p_{\text{abs}}$ | 100 k ₳ (within-pool pledge ratio = 2 %) |
| Total-stake saturation level $\nu = \sigma/z_0$ | 0.065 (6.5 % of V1 saturation) |
| Pledge saturation level $\pi = p/z_0$ | 0.0013 (0.13 % of V1 saturation) |
| $L \cdot \pi$ | 0.13 — greater than $\nu = 0.065$, so does **not** bind |
| $\nu' = \min(\nu, 1, L\cdot\pi)$ | **$\nu$** — cap inactive |
...
| Pledge $p_{\text{abs}}$ | 10.5 k ₳ (0.07 % ratio — stake-weighted median per POL.O2.F1) |
| $\nu = \sigma / z_0$ | 0.195 (19.5 % of V1 saturation) |
| $\pi = p / z_0$ | 0.000136 (0.014 % of V1 saturation) |
| $L \cdot \pi$ | 0.0136 — **less than** $\nu = 0.195$, **cap binds** |
| $\nu' = \min(\nu, 1, L\cdot\pi)$ | $\nu' = L \cdot \pi = $ **0.0136** (saturation level clipped to 1.4 % of V1) |
| Pool reward | $\nu'/\nu = 0.0136 / 0.195 \approx$ **7 % of V1 baseline** — 93 % cut |

**Scenario C — Saturated high-pledge pool.**

| Quantity | Value |
| --- | --- |
| Total stake $\sigma_{\text{abs}}$ | 77 M ₳ (at saturation) |
| Pledge $p_{\text{abs}}$ | 1 M ₳ (within-pool pledge ratio = 1.3 %) |
| $\nu = \sigma / z_0$ | 1.0 (at V1 saturation) |
| $\pi = p / z_0$ | 0.013 (1.3 % of V1 saturation) |
| $L \cdot \pi$ | 1.3 — **greater than** 1, does not bind |
| $\nu' = \min(\nu, 1, L\cdot\pi)$ | $\nu' = $ **1.0** (V1 saturation cap binds) |
| Pool reward | V1 baseline, unchanged |

**The core consequence.** For a given $L$, the reform partitions the pool population by a single threshold on the **within-pool pledge ratio** $p/\sigma = \pi/\nu$: at or above $1/L$, CIP-0050 is inactive; below it, the effective saturation level $\nu'$ is clipped to $L \cdot \pi$ and the pool reward scales with the clip ratio $\nu'/\nu = L \cdot (p/\sigma)$. Scenarios A and C both illustrate "cap does not bind" but for different reasons — A is compliant on pledge ratio; C is bounded by V1 saturation before the pledge cap can bite.
