...
| CIP-0050.S2.F3 | [B] | **The bet that operators will pledge more is contradicted by the opportunity-cost data.** POL.O2.F2: pledge yield is 0.68 %/yr at best vs 2.3 %/yr on passive delegation. POL.O5.F3: 42 of 48 saturation-scale MPOs already forfeit the bonus today. CIP-0050 shifts the rules but not the passive-delegation-vs-pledge comparison — and adds opportunity cost without flipping the dominance relation | §3.2 |
...
| CIP-0050.S3.F2 | [R] | **§3.1 viability risk.** Retail small-pool operators with attracted delegation and low pledge (POL.O6.F2: 78 % of single-pool stake zero-pledge) see reward clipped — and with the fee split unchanged, *both* operator take and delegator ROS fall proportionally. Without a companion fee-layer viability floor, CIP-0050 accelerates small-pool attrition | §3.3 |
...
- [2. Mechanism](#2-mechanism)
  ...
  - [2.4. Visualising the σ' cap — the "pledge cliff" at the 1/L threshold](#24-visualising-the-cap-the-pledge-cliff-at-the-1l-threshold)
...
- [3. Limits of CIP-0050 as a standalone proposal](#3-limits-of-cip-0050-as-a-standalone-proposal)
  ...
  - [3.3. S3 — Entity-level gap and small-pool viability risk](#33-s3-entity-level-34-gap-and-31-viability-risk)
...
| **POL.O5.F3 — 42 of 48 saturation-scale MPOs are zero-pledge on pledge** | The population that *could* respond to CIP-0050 by increasing pledge has already shown it does not choose that option under current rules. CIP-0050 shifts the rules but the opportunity-cost calculus does not flip | — |
| **POL.O6.F2 — 78 % of single-pool stake is zero-pledge (pledge ratio < 2 %). Compliant + exemplary hold 5.8 % — economically negligible** | Retail single-pool operators — the population V2 §3.1 targets — empirically do not pledge. CIP-0050 at L=100 clips this population's reward, accelerating attrition | — |
...
*C50.2.1 — Three competing caps for the median retail pool ($\sigma = 15$ M, $p = 10.5$ k): the new pledge-leverage cap **$L \cdot p = 1.05$ M ADA** wins the min, slicing the reward-eligible stake to **7 %** of the pool's actual size.*
...
*C50.2.2 — Reward fraction preserved across the three scenarios at $L = 100$: compliant (A) and saturated (C) pools keep **100 %**; the zero-pledge median retail pool (B) drops to **~7 %** — a binary line at $\pi = 1/L$.*
...
*C50.2.3 — Decision flow for the three-way min computing $\sigma'$: each pool falls into one of three regimes — **Clipped** (pledge cap binds), **Unaffected** (actual stake binds), or **V1 saturation** ($1/k$ binds, CIP-0050 is a no-op).*
...
*C50.2.4 — Reward fraction preserved as a function of within-pool pledge ratio $\pi$ for four leverage values: linear ramp up to the cliff at $\pi = 1/L$, flat at 100 % above. Median-pool marker at $\pi = 0.07\,\%$ falls in the clipped regime for every $L \geq 10$.*
...
*C50.2.5 — Ramp impact on the median retail pool ($\pi = 0.07\,\%$): reward preserved drops from **~100 %** at activation ($L=10\,000$) to **~7 %** at the long-run target ($L=100$) and **~1.75 %** at $L=25$.*
...
*C50.2.6 — Minimum pledge needed per tier to escape clipping: a Healthy pool requires **150 k ADA** at $L=100$ and **1.5 M ADA** at $L=10$ — linear in pool stake and in $1/L$.*
...
**What can an operator do?** A zero-pledge operator facing the cap has four response paths. Using the **Healthy-tier 15 M ₳ pool** as the reference (10.5 k ₳ pledge = 0.07 % ratio):
...
*C50.2.7 — Fleet reward-eligible capacity vs pool count for an operator with 1 M ADA pledge: capacity plateaus at **$L \cdot P$ = 100 M ADA** once $L \cdot p < 1/k$, making pool-splitting revenue-neutral.*
...
*C50.2.9 — Annual yield on a 10 000 ADA stake across the three scenarios: compliant pools preserve **~227 ADA/yr**; the zero-pledge median pool collapses to **~16 ADA/yr** (a **~93%** cut).*
...
*C50.2.10 — Stake-weighted partition of the **21.57 B ADA** productive mainnet at $L = 100$: ~**18 B ADA** (84 %) sits in pools that would be clipped or collapsed.*
...
| Retail, pledge < 1 % (zero-pledge) | ~94 % × 17 B ≈ 16.0 B ₳ | Typically 0.07–1 % | **Clipped 0 – 93 %** |

**Approximately 18 B ₳ of productive stake (~83 %)** — Custodial-by-extraction (2.04 B clipped to ~0) plus Retail zero-pledge (16 B clipped 0–93 %) — sits in pools where CIP-0050 at L=100 would bind and clip. A network-wide shock under any non-gradual ramp.
...
> **Finding CIP-0050.S2.F3 [B] — The "operators will pledge more" bet is contradicted by the opportunity-cost data.** POL.O2.F2: pledge yield is **0.68 %/yr** at best vs **~2.3 %/yr** on passive delegation — pledge is structurally dominated by its alternative. POL.O5.F3: 42 of 48 saturation-scale MPOs already forfeit the pledge bonus today (~40.2 M ₳/yr aggregate) rather than lock capital into a dominated position. CIP-0050 changes the rules but not the dominance relation — the population that has *chosen* not to pledge continues to face the same opportunity-cost argument against pledging. The bet the reform rests on is not empirically supported.

**Composite reading of S2.** CIP-0050 penalises two populations that cannot respond (custodial + sub-saturation retail) and one that has *chosen* not to respond (saturation-scale MPOs). The discrimination is by capital capability, not by operator quality or network contribution. A pool can produce blocks reliably, serve delegators well, and operate with integrity — and still be clipped to a fraction of its reward if its operator does not have the capital to self-pledge above 1 %.
