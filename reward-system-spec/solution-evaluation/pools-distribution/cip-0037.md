...
| CIP-0037.S1.F2 | [D] | **20 % floor protects the smallest pools.** $\text{sat}(0) = 0.2 \cdot \text{orig\_sat} \approx 13.5$ M ₳ — a zero-pledge pool still keeps 20 % of the V1 saturation cap. Sub-production and Sub-viable tiers (σ ≪ 13.5 M) see no clipping under zero pledge | §2.2, §2.3, §3.1 |
...
| CIP-0037.S2.F4 | [B] | **The "operators will pledge more" bet is contradicted by POL.O2.F2 + POL.O4.F3.** Pledge yield is 0.68 %/yr at best vs ~2.3 %/yr passive delegation (POL.O2.F2). 41 of 48 capital-sufficient MPOs already forfeit the bonus (POL.O4.F3). CIP-0037 reshapes the rules but does not flip the dominance relation — the population that has *chosen* not to pledge continues to face the same opportunity cost | §3.2 |
...
| **POL.O4.F1 / POL.O4.F2 — 85 MPO entities operate 901 pools holding 75.4 % of participating stake; 48 are capital-sufficient** | Entity-level concentration metric. CIP-0037's ceiling regime leaves large-pledge entities unconstrained — custodial-by-pledge segment not addressed | — |
| **POL.O4.F3 — 41 of 48 capital-sufficient MPOs forfeit the pledge bonus today** | The population that *could* respond to CIP-0037 by increasing pledge has already shown it does not choose that option under current rules | — |
| **POL.O5.F2 — 78 % of single-pool stake has pledge ratio < 2 %** | Retail single-pool operators — the population V2 §3.1 targets — empirically do not pledge above the CIP-0037 floor-escape threshold. Their pool reward is clipped to 20 % of the V1 cap | — |
...
The reward-eligible stake becomes $\sigma' = \min(\sigma, \text{sat}(p))$; the reward curve then applies to $\sigma'$: $\hat f' = \bar p \cdot P_{\max} \cdot E(\pi, \nu')$ with $\nu' = \sigma' / z_0$. The operator/member split is unchanged.
...
| **Price dependence** | $p_{100\%}$ is absolute ADA; fiat-denominated opportunity cost shifts with ADA/$$ | Structural |
...
| Sub-production | 500 K | 13.49 M | 500 K | 100 % (unchanged) |
| Sub-viable | 2 M | 13.49 M | 2 M | 100 % (unchanged) |
...
*Reading the table.* The 20 % floor protects pools with σ ≤ 13.49 M — that is, **all tiers up to and including Sub-viable** and the lower half of the Healthy tier. Above 13.49 M, zero-pledge pools are clipped in proportion to (13.49 M / σ). A zero-pledge Saturated pool (77 M) keeps ~18 % of its V1 reward.
...
> **Finding CIP-0037.S1.F2 [D] — 20 % floor protects the smallest pools.** $\text{sat}(0) = 0.2 \cdot \text{orig\_sat} \approx 13.49$ M ₳. Any pool with total stake ≤ 13.49 M is unaffected under zero pledge — which covers Dormant, Sub-production, Sub-viable, and the lower half of the Healthy tier in the canonical nine-tier taxonomy. Unlike CIP-0050, the instrument does not zero out hollow pools; it caps them at 20 % of the V1 saturation.
...
> **Finding CIP-0037.S2.F4 [B] — The "operators will pledge more" bet is contradicted by POL.O2.F2 + POL.O4.F3.** Same as for CIP-0050: pledge yield 0.68 %/yr is structurally dominated by passive-delegation yield 2.3 %/yr; 41 of 48 capital-sufficient MPOs already forfeit the bonus today. CIP-0037 changes the saturation curve but not the opportunity-cost comparison that produces the current non-pledge equilibrium.

**Composite reading of S2.** The 20 % floor softens CIP-0037 at the very bottom of the distribution (Sub-viable and below) — a real advantage over CIP-0050's hard break. But from the Healthy tier upward, CIP-0037 clips **even more harshly than a naive reading of the floor suggests**, because the pool-size-independent floor-exit threshold means mid-tier retail pools need absolute pledge levels most operators do not have. The discrimination is by capital capability, same as CIP-0050.
...
> **Finding CIP-0037.S3.F2 [R] — Price-dependence breaks V2 §4.3 price-robustness.** $p_{100\%} = 500\,000$ ADA is **absolute**, not a ratio. The fiat cost of reaching full pledge shifts directly with ADA/$$: at $0.10 it costs $50 k; at $1.00 it costs $500 k; at $5.00 it costs $2.5 M. Operator behaviour responds to fiat cost; the calibration needs to be re-peg'd on material price moves. CIP-0050's dimensionless $L$ is structurally price-invariant by construction.
