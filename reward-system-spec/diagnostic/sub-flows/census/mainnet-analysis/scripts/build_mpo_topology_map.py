...
<head>
<meta charset="utf-8">
<title>Cardano Infrastructure Topology &mdash; Interactive Map</title>
...
<style>
  :root { --infared: #E52321; --dawn: #EC641D; --ink: #1a1a1a; --grey: #888; --line: #ddd; }
  html, body { margin: 0; padding: 0; height: 100%; overflow: hidden;
               font-family: -apple-system, system-ui, sans-serif; color: var(--ink); }
  #topbar { position: absolute; left: 0; right: 0; top: 0; height: 56px; padding: 8px 14px;
            background: #000; color: #fff; box-sizing: border-box; display: flex;
            align-items: center; gap: 18px; border-bottom: 3px solid var(--infared); z-index: 1000; }
  #topbar h1 { font-size: 14px; margin: 0; color: #fff; letter-spacing: 0.02em; }
  #topbar .sub { color: #888; font-size: 11px; }
  #topbar .ctrl { display: flex; gap: 4px; align-items: center; }
  #topbar label { font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; color: #888; }
  .seg { display: inline-flex; background: #222; border-radius: 4px; overflow: hidden; }
  .seg button { background: none; border: 0; color: #aaa; padding: 5px 10px; font-size: 11px;
                cursor: pointer; font-family: inherit; }
  .seg button.on { background: var(--infared); color: #fff; }
  .seg button:hover:not(.on) { background: #333; color: #fff; }
  #panel { position: absolute; left: 0; top: 56px; bottom: 0; width: 280px; padding: 12px 14px;
           background: #fafafa; border-right: 1px solid var(--line); overflow-y: auto;
           font-size: 12px; box-sizing: border-box; }
  #map { position: absolute; left: 280px; right: 0; top: 56px; bottom: 0; }
  #panel h2 { font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em;
              margin: 12px 0 6px; color: #555; }
  #panel .kpi { font-size: 22px; font-weight: 700; line-height: 1.1; color: var(--infared); }
  #panel .sub { font-size: 11px; color: #666; }
  #panel ul { list-style: none; padding: 0; margin: 4px 0; }
  #panel li { display: flex; align-items: center; padding: 3px 0; gap: 6px;
              cursor: pointer; user-select: none; border-radius: 3px; }
  #panel li.off { opacity: 0.35; }
  #panel li .sw { width: 12px; height: 12px; border-radius: 50%; border: 1px solid #00000022;
                  flex-shrink: 0; }
  #panel li .pct { margin-left: auto; font-variant-numeric: tabular-nums; color: #666;
                   font-size: 11px; }
...
<div id="topbar">
  <div>
    <h1>Cardano Infrastructure Topology</h1>
    <div class="sub">Stake-weighted relay footprint &mdash; <span id="snapshot"></span></div>
  </div>
  <div class="ctrl">
    <label>Cohort</label>
    <div class="seg" id="cohortSeg">
      <button data-v="all" class="on">All</button>
      <button data-v="mpo">MPO only</button>
      <button data-v="spo">Single-pool</button>
    </div>
  </div>
  <div class="ctrl">
    ...
    <div class="seg" id="popSeg">
      <button data-v="all" class="on">All</button>
      <button data-v="productive">Productive (≥3M)</button>
    </div>
  </div>
  <div class="ctrl">
    <label>Aggregation</label>
    <div class="seg" id="aggSeg">
      <button data-v="pool" class="on">Per pool</button>
      <button data-v="entity">Per entity</button>
    </div>
  </div>
  <div class="ctrl">
    <label>Colour by</label>
    <div class="seg" id="colorSeg">
      <button data-v="provider" class="on">Provider</button>
      <button data-v="tier">Tier</button>
      <button data-v="cohort">Cohort</button>
      <button data-v="saturation">Saturation</button>
    </div>
  </div>
  <div class="ctrl">
    <label>Size / heat by</label>
    <div class="seg" id="metricSeg">
      <button data-v="stake" class="on">Stake</button>
      <button data-v="saturation">Saturation %</button>
      <button data-v="blocks">Blocks/epoch</button>
    </div>
  </div>
  <div class="ctrl">
    <label>Layer</label>
    <div class="seg" id="layerSeg">
      <button data-v="pins" class="on">Pins</button>
...
<div id="panel">
  <h2>Current view</h2>
  <div class="kpi" id="kpiCount">&mdash;</div>
  <div class="sub" id="kpiStake">&mdash;</div>
  <div class="sub" id="kpiTop">&mdash;</div>
  <h2 id="filterHeading">Filter by provider</h2>
  <div class="sub" style="margin-bottom:4px">Click a row to toggle</div>
  <ul id="filterList"></ul>
  <h2>Entities in view</h2>
  <div class="sub" style="margin-bottom:4px">Click a row to fly to it on the map</div>
  <div id="entityPanel"></div>
  <h2>Reading the map</h2>
  <div class="sub">
    Pin <b>size</b> scales with the chosen metric (stake, saturation %, or
    expected blocks per epoch). Pin <b>colour</b> shows the chosen dimension.
    Click any pin for full entity / pool details.
    <br/><br/>
    <b>Population</b>: <i>All</i> shows every active registered pool.
    <i>Productive (≥3M)</i> filters to the diagnostic's productive segment
    (POL.O4.F2: 27% of pools, 96.6% of stake) &mdash; the consensus-carrying
    population that any reform of <i>k</i>, the pledge curve, or the
    saturation cap actually moves.
    <br/><br/>
    <b>Colour by &rarr; Saturation</b> uses the diagnostic's canonical
    9-tier taxonomy with z<sub>0</sub> = supply / k. Tiers ordered from
    Zero-stake / Dormant / Sub-block / Sub-reliable through Healthy / Large
    healthy / Near-saturation / Saturated / Oversaturated.
    <br/><br/>
    <b>Aggregation &rarr; Per entity</b> collapses each operator to its
    largest single relay cluster (Coinbase &rarr; Frankfurt, Upbit &rarr;
    Seoul, Cardano Foundation &rarr; Zurich, ...). The entity tier is the
    <i>average per-pool</i> tier, which is the meaningful operator-level read.
  </div>
</div>
<div id="map"></div>
<script>
...
const state = {
  cohort: 'all',       // all | mpo | spo
  pop: 'all',          // all | productive
  agg: 'pool',         // pool | entity
  color: 'provider',   // provider | tier | cohort | saturation
  metric: 'stake',     // stake | saturation | blocks
  layer: 'pins',       // pins | heat | both
  enabledKeys: new Set(),
};
...
// hook segment buttons
['cohortSeg','popSeg','aggSeg','colorSeg','metricSeg','layerSeg'].forEach(id => {
  const root = document.getElementById(id);
  root.querySelectorAll('button').forEach(b => {
    b.onclick = () => {
      root.querySelectorAll('button').forEach(x => x.classList.remove('on'));
      b.classList.add('on');
      const key = {cohortSeg:'cohort', popSeg:'pop', aggSeg:'agg',
                   colorSeg:'color', metricSeg:'metric', layerSeg:'layer'}[id];
      state[key] = b.dataset.v;
      // reset filter only when colour dimension, cohort, or population changes
      if (['cohortSeg','popSeg','colorSeg'].includes(id)) state.enabledKeys = new Set();
      rebuild();
    };
  });
});
...
function metricValue(r) {
  if (state.metric === 'saturation') return r.saturation_pct;
  if (state.metric === 'blocks') return r.expected_blocks_per_epoch;
  return r.stake_ada;
}

function metricLabel(v) {
  if (state.metric === 'saturation') return v.toFixed(1) + '%';
  if (state.metric === 'blocks') return v.toFixed(2) + ' blk/epoch';
...
function heatWeight(r) {
  const m = metricValue(r);
  if (state.metric === 'saturation') return Math.min(20, Math.sqrt(m));
  if (state.metric === 'blocks') return Math.min(20, Math.sqrt(m * 100));
  return Math.sqrt(m);
}
...
function renderEntityPanel(filtered, totalAda) {
  ...
  panel.innerHTML = '';
  const ents = entitiesFromCurrentView(filtered)
...
function flyToEntity(ent) {
  ...
  // pop a transient marker with the entity info, since the matching cluster
  // pin may be inside an unexpanded cluster
  ...
  m.bindPopup(
    ...
    + (ent.pool_count > 1 ? '  <span style="color:#888;font-size:11px">[' + ent.pool_count + ' pool' + (ent.pool_count > 1 ? 's' : '') + ']</span>' : '')
...
function rebuild() {
  ...
  document.getElementById('kpiStake').textContent =
    (filteredAda / 1e9).toFixed(2) + 'B ADA  ('
    + (100 * filteredAda / totalAda).toFixed(1) + '% of view)';
  // KPI line 3 changes meaning with the metric
  let kpi3 = '';
  if (state.metric === 'saturation' || state.color === 'saturation') {
