...
OUTPUT_FILENAME = "appendixA_548-583.txt" # File to save the report to
...
rewards_in_window AS (
    ...
    FROM reward
    WHERE type = 'leader' AND earned_epoch >= (SELECT MIN(epoch_no) FROM epoch_range)
    GROUP BY pool_id
),
...
spo_voting_activity AS (
    ...
    WHERE vp.voter_role = 'SPO'
      AND b.epoch_no >= (SELECT MIN(epoch_no) FROM epoch_range)
),
...
def analyze_and_report_pools():
    ...
    try:
        ...
        one_year_ago = pd.Timestamp.now('UTC') - timedelta(days=365)
