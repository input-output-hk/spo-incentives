...
# Pin: read the published Appendix A (1,305 Inactive pools) directly so the
# chart's "Excluding N Inactive Pools" title matches the report.
...
def plot_pool_size_distribution():
    ...
    try:
        ...
        # MAX(epoch_no), so re-running today reproduces the report's chart domain.
        latest_epoch = ANALYSIS_END_EPOCH
        ...
        fig.tight_layout()
        output_filename = 'pool_size_distribution-pinned.png'
