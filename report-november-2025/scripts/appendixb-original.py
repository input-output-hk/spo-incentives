...
OUTPUT_CSV_FILE = "analysis_results_filtered.csv"
...
WITH target_epoch AS (
  SELECT ((SELECT MAX(epoch_no) FROM epoch_stake) - %s) AS epoch_no
),
...
epoch_blocks AS (
  ...
  SELECT id, hash_id FROM (
      SELECT id, hash_id, ROW_NUMBER() OVER (PARTITION BY hash_id ORDER BY registered_tx_id DESC) as rn
      FROM pool_update
...
def run_query_worker(offset, min_lovelace, max_lovelace, range_str):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT MAX(epoch_no) FROM epoch_stake")
            latest_epoch = cur.fetchone()[0]
            target_epoch = latest_epoch - offset
