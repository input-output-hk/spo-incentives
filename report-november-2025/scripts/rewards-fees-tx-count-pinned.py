...
WITH latest_epoch AS (
    -- Pin: anchor the 73-epoch window's upper bound to 591 (the published
    -- chart's end epoch) instead of querying db-sync's current max.
    SELECT 591 as max_no
),
epoch_range AS (
...
def create_rewards_fees_chart(df, start_epoch, end_epoch, avg_rewards, avg_fees, avg_tx_count):
    ...
    output_filename = f"rewards_fees_tx_count_last_73_epochs-pinned.png"
